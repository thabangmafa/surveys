...
<?php
    ...
    echo heading('Edit question', '/images/surveys/questions.png');
    echo rms_form_open_multipart('surveys/update_question/' . $survey_id . '/' . $section_id, array('id' => 'surveyform', 'name' => 'surveyform'));
    echo rms_form_input('QUESTION', 'Question', 'This field is mandatory.', 'text', $question[0]['QUESTION'], 'size="128" class="validate[required]"');
    echo rms_form_dropdown('OPTIONCOUNT', 'Option count', 'This field is mandatory.', 'OPTIONCOUNT', $count_options, $question[0]['OPTIONCOUNT'], 'id="OPTIONCOUNT" class="validate[required]"');
    echo rms_form_dropdown('QUESTIONTYPE_ID', 'Question type', 'This field is mandatory.', 'QUESTIONTYPE_ID', $type_options, $question[0]['QUESTIONTYPE_ID'], 'id="QUESTIONTYPE_ID" class="validate[required]"');
    echo rms_form_dropdown('ISREQUIRED', 'Is this question required?', 'This field is mandatory.', 'ISREQUIRED', $yesno_options, $question[0]['ISREQUIRED'], 'id="ISREQUIRED" class="validate[required]"');
    echo rms_form_hidden('SURVEY_ID', $survey_id);
    echo rms_form_hidden('SECTION_ID', $section_id);
    echo rms_form_hidden('ID', $question[0]['ID']);
